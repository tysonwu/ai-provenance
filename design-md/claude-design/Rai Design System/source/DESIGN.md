...
### Dark Theme (Financial Architect Overlay)

The light theme remains pure IBM Carbon.  
Dark mode intentionally shifts to the Bloomberg-inspired "Financial Architect" system from `rai/ui/docs/DESIGN.md`: high-contrast data surfaces, tonal layering over visible lines, and restrained cobalt highlights.

- **Background**: Ink black (`#121316`). `--cds-background`
- **Layer 01**: Structural surface (`#1b1e24`). `--cds-layer-01`
- **Layer 01 Hover**: Interaction hover (`#252a32`). `--cds-layer-01-hover`
- **Layer 02**: Deep elevated canvas (`#0f1115`). `--cds-layer-02`
- **Text Primary**: High-contrast off-white (`#eef0f7`). `--cds-text-primary`
- **Text Secondary**: Muted slate (`#b4b8c8`). `--cds-text-secondary`
- **Border Subtle**: Ghost separator (`#4a5162`) at low opacity only. `--cds-border-subtle`
- **Interactive Accent**: Market blue (`#6f95ff`) with hover (`#9db5ff`). `--cds-link-primary`, `--cds-link-primary-hover`
- **Success/Error**: Financial green (`#43c59c`) and risk red (`#ff8e8e`) for clear state semantics
- **Masthead/Footer**: Translucent dark chrome with blur, not hard bars

#### Dark-Mode Behavior Rules

- No-line rule in dark mode: avoid explicit section borders; separate content with tone shifts.
- Use ghost outlines only when needed for accessibility: 15% `outline-variant`.
- Preserve IBM spacing, type scale, and component geometry (0px corners); only dark tonal semantics change.
- Keep underlined-input interaction model from Carbon, but tune contrast for dark surfaces.
